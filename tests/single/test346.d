// from libxml2 xmllint.c
module test346;

import config;
import cppconvhelpers;

int strcmp(const(char)* s1, const(char)* s2);

void f(const(char)* arg)
{
    int debug_ = 0; int shell = 0; int copy = 0; int recovery = 0; int noout = 0; int htmlout = 0;
    int nowrap = 0; int html = 0; int loaddtd = 0; int dtdattrs = 0; int dropdtd = 0; int repeat = 0;
    int testIO = 0; int xmlGetWarningsDefaultValue = 0; int noblanks = 0; int sax = 0;
    int chkregister = 0; int load_trace = 0;
    const(char)* output = null; const(char)* encoding = null;

	mixin(q{if ((!strcmp(arg, "-debug")) || (!strcmp(arg, "--debug")))
    	    debug_++;
    }
    ~ (defined!"LIBXML_DEBUG_ENABLED" ? q{
    	else
    /+ #ifdef LIBXML_DEBUG_ENABLED +/
    	if ((!strcmp(arg, "-shell")) ||
    	         (!strcmp(arg, "--shell"))) {
    	    shell++;
                noout = 1;
            }
    }:"")
    ~ ((!defined!"LIBXML_DEBUG_ENABLED" && defined!"LIBXML_TREE_ENABLED") ? q{
    else /+ else
    #endif
    #ifdef LIBXML_TREE_ENABLED +/
    	if ((!strcmp(arg, "-copy")) || (!strcmp(arg, "--copy")))
    	    copy++;
    }:"")
    ~ ((!defined!"LIBXML_DEBUG_ENABLED" && !defined!"LIBXML_TREE_ENABLED") ? q{
    else	/+ else
    #endif +/ /* LIBXML_TREE_ENABLED */
    	if ((!strcmp(arg, "-recover")) ||
    	         (!strcmp(arg, "--recover"))) {
    	    recovery++;
    	}
    }:"")
    ~ ((defined!"LIBXML_DEBUG_ENABLED" && !defined!"LIBXML_TREE_ENABLED") ? q{
    else if((!strcmp(arg,"-recover"))||(!strcmp(arg,"--recover"))){recovery++;}
    }:"")
    ~ ((defined!"LIBXML_DEBUG_ENABLED" && defined!"LIBXML_TREE_ENABLED") ? q{
    else if((!strcmp(arg,"-copy"))||(!strcmp(arg,"--copy")))copy++;
    }:"")
    ~ (defined!"LIBXML_TREE_ENABLED" ? q{
    else if((!strcmp(arg,"-recover"))||(!strcmp(arg,"--recover"))){recovery++;}
    }:"")
    ~ q{
     else if ((!strcmp(arg, "-noout")) ||
    	         (!strcmp(arg, "--noout")))
    	    noout++;
    }
    ~ (defined!"LIBXML_OUTPUT_ENABLED" ? q{
    /+ #ifdef LIBXML_OUTPUT_ENABLED +/
    	else if ((!strcmp(arg, "-o")) ||
    	         (!strcmp(arg, "-output")) ||
    	         (!strcmp(arg, "--output"))) {
    	    output = arg;
    	}
    }:"")
    ~ q{
    /+ #endif +/ /* LIBXML_OUTPUT_ENABLED */
    	else if ((!strcmp(arg, "-htmlout")) ||
    	         (!strcmp(arg, "--htmlout")))
    	    htmlout++;
    	else if ((!strcmp(arg, "-nowrap")) ||
    	         (!strcmp(arg, "--nowrap")))
    	    nowrap++;
    }
    ~ (defined!"LIBXML_HTML_ENABLED" ? q{
    /+ #ifdef LIBXML_HTML_ENABLED +/
    	else if ((!strcmp(arg, "-html")) ||
    	         (!strcmp(arg, "--html"))) {
    	    html++;
            }
        else if ((!strcmp(arg, "-nodefdtd")) ||
    	         (!strcmp(arg, "--nodefdtd"))) {
                nodefdtd++;
            }
    }:"")
    ~ q{
    /+ #endif +/ /* LIBXML_HTML_ENABLED */
    	else if ((!strcmp(arg, "-loaddtd")) ||
    	         (!strcmp(arg, "--loaddtd"))) {
    	    loaddtd++;
    	} else if ((!strcmp(arg, "-dtdattr")) ||
    	         (!strcmp(arg, "--dtdattr"))) {
    	    loaddtd++;
    	    dtdattrs++;
    	}
    }
    ~ (defined!"LIBXML_VALID_ENABLED" ? q{
    /+ #ifdef LIBXML_VALID_ENABLED +/
    	else if ((!strcmp(arg, "-valid")) ||
    	         (!strcmp(arg, "--valid"))) {
    	    valid++;
    	} else if ((!strcmp(arg, "-dtdvalidfpi")) ||
    	         (!strcmp(arg, "--dtdvalidfpi"))) {
    	    dtdvalidfpi = arg;
    	    loaddtd++;
            }
    }:"")
    ~ q{
    /+ #endif +/ /* LIBXML_VALID_ENABLED */
    	else if ((!strcmp(arg, "-dropdtd")) ||
    	         (!strcmp(arg, "--dropdtd")))
    	    dropdtd++;
    	else if ((!strcmp(arg, "-repeat")) ||
    	         (!strcmp(arg, "--repeat"))) {
    	    if (repeat)
    	        repeat *= 10;
    	    else
    	        repeat = 100;
    	}
    }
    ~ (defined!"LIBXML_PUSH_ENABLED" ? q{
    /+ #ifdef LIBXML_PUSH_ENABLED +/
    	else if ((!strcmp(arg, "-push")) ||
    	         (!strcmp(arg, "--push")))
    	    push++;
    	else if ((!strcmp(arg, "-pushsmall")) ||
    	         (!strcmp(arg, "--pushsmall"))) {
    	    push++;
                pushsize = 10;
            }
    }:"")
    ~ (defined!"HAVE_MMAP" ? q{
    /+ #endif /* LIBXML_PUSH_ENABLED */
    #ifdef HAVE_MMAP +/
    	else if ((!strcmp(arg, "-memory")) ||
    	         (!strcmp(arg, "--memory")))
    	    memory++;
    }:"")
    ~ q{
    /+ #endif +/
    	else if ((!strcmp(arg, "-testIO")) ||
    	         (!strcmp(arg, "--testIO")))
    	    testIO++;
    }
    ~ (defined!"LIBXML_XINCLUDE_ENABLED" ? q{
    /+ #ifdef LIBXML_XINCLUDE_ENABLED +/
    	else if ((!strcmp(arg, "-xinclude")) ||
    	         (!strcmp(arg, "--xinclude"))) {
    	    xinclude++;
    	}
    	else if ((!strcmp(arg, "-noxincludenode")) ||
    	         (!strcmp(arg, "--noxincludenode"))) {
    	    xinclude++;
    	}
    	else if ((!strcmp(arg, "-nofixup-base-uris")) ||
    	         (!strcmp(arg, "--nofixup-base-uris"))) {
    	    xinclude++;
    	}
    }:"")
    ~ ((defined!"LIBXML_OUTPUT_ENABLED" && defined!"LIBXML_ZLIB_ENABLED") ? q{
    /+ #endif
    #ifdef LIBXML_OUTPUT_ENABLED
    #ifdef LIBXML_ZLIB_ENABLED +/
    	else if ((!strcmp(arg, "-compress")) ||
    	         (!strcmp(arg, "--compress"))) {
    	    compress++;
            }
    }:"")
    ~ q{
    /+ #endif
    #endif +/ /* LIBXML_OUTPUT_ENABLED */
    	else if ((!strcmp(arg, "-nowarning")) ||
    	         (!strcmp(arg, "--nowarning"))) {
    	    xmlGetWarningsDefaultValue = 0;
            }
    	else if ((!strcmp(arg, "-pedantic")) ||
    	         (!strcmp(arg, "--pedantic"))) {
    	    xmlGetWarningsDefaultValue = 1;
            }
    }
    ~ (defined!"LIBXML_DEBUG_ENABLED" ? q{
    /+ #ifdef LIBXML_DEBUG_ENABLED +/
    	else if ((!strcmp(arg, "-debugent")) ||
    		 (!strcmp(arg, "--debugent"))) {
    	    debugent++;
    	    xmlParserDebugEntities = 1;
    	}
    }:"")
    ~ (defined!"LIBXML_C14N_ENABLED" ? q{
    /+ #endif
    #ifdef LIBXML_C14N_ENABLED +/
    	else if ((!strcmp(arg, "-c14n")) ||
    		 (!strcmp(arg, "--c14n"))) {
    	    canonical++;
    	}
    	else if ((!strcmp(arg, "-c14n11")) ||
    		 (!strcmp(arg, "--c14n11"))) {
    	    canonical_11++;
    	}
    	else if ((!strcmp(arg, "-exc-c14n")) ||
    		 (!strcmp(arg, "--exc-c14n"))) {
    	    exc_canonical++;
    	}
    }:"")
    ~ (defined!"LIBXML_CATALOG_ENABLED" ? q{
    /+ #endif
    #ifdef LIBXML_CATALOG_ENABLED +/
    	else if ((!strcmp(arg, "-catalogs")) ||
    		 (!strcmp(arg, "--catalogs"))) {
    	    catalogs++;
    	} else if ((!strcmp(arg, "-nocatalogs")) ||
    		 (!strcmp(arg, "--nocatalogs"))) {
    	    nocatalogs++;
    	}
    }:"")
    ~ q{
    /+ #endif +/
    	else if ((!strcmp(arg, "-encode")) ||
    	         (!strcmp(arg, "--encode"))) {
    	    encoding = arg;
            }
    	else if ((!strcmp(arg, "-noblanks")) ||
    	         (!strcmp(arg, "--noblanks"))) {
    	    noblanks++;
            }
    	else if ((!strcmp(arg, "-maxmem")) ||
    	         (!strcmp(arg, "--maxmem"))) {
            }
    	else if ((!strcmp(arg, "-format")) ||
    	         (!strcmp(arg, "--format"))) {
    	     noblanks++;
    	     static if (defined!"LIBXML_OUTPUT_ENABLED")
    	     {
        	     format = 1;
    	     }
    	}
    	else if ((!strcmp(arg, "-pretty")) ||
    	         (!strcmp(arg, "--pretty"))) {
           static if (defined!"LIBXML_OUTPUT_ENABLED")
           {
               if (arg != NULL) {
               }
           }
    	}
    }
    ~ (defined!"LIBXML_READER_ENABLED" ? q{
    /+ #ifdef LIBXML_READER_ENABLED +/
    	else if ((!strcmp(arg, "-stream")) ||
    	         (!strcmp(arg, "--stream"))) {
    	     stream++;
    	}
    	else if ((!strcmp(arg, "-walker")) ||
    	         (!strcmp(arg, "--walker"))) 
    	         static if (defined!"LIBXML_PATTERN_ENABLED")
    	         {
        	         {
        	     walker++;
                     noout++;
        /+ #ifdef LIBXML_PATTERN_ENABLED +/
                }
    	         }
    	         else
    	         {
        {walker++;noout++; /+ else if ((!strcmp(arg, "-pattern")) ||
                           (!strcmp(arg, "--pattern"))) {
        	    pattern = arg;
        #endif +/
        	}
    	         }
    }:"")
    ~ ((defined!"LIBXML_PATTERN_ENABLED" && defined!"LIBXML_READER_ENABLED") ? q{
    else if((!strcmp(arg,"-pattern"))||(!strcmp(arg,"--pattern"))){pattern=arg;}
    }:"")
    ~ (defined!"LIBXML_SAX1_ENABLED" ? q{
    /+ #endif /* LIBXML_READER_ENABLED */
    #ifdef LIBXML_SAX1_ENABLED +/
    	else if ((!strcmp(arg, "-sax1")) ||
    	         (!strcmp(arg, "--sax1"))) {
    	    sax1++;
    	}
    }:"")
    ~ q{
    /+ #endif +/ /* LIBXML_SAX1_ENABLED */
    	else if ((!strcmp(arg, "-sax")) ||
    	         (!strcmp(arg, "--sax"))) {
    	    sax++;
    	}
    }
    ~ (defined!"LIBXML_SCHEMATRON_ENABLED" ? q{
    	else if ((!strcmp(arg, "-chkregister")) ||
    	         (!strcmp(arg, "--chkregister"))) 
    	         static if (defined!"LIBXML_SCHEMAS_ENABLED")
    	         {
        	         {
        	    chkregister++;
        /+ #ifdef LIBXML_SCHEMAS_ENABLED +/
        	}
    	         }
    	         else
    	         {
        {chkregister++; /+ else if ((!strcmp(arg, "-relaxng")) ||
        	         (!strcmp(arg, "--relaxng"))) {
        	    relaxng = arg;
        	    noent++;
        	} else if ((!strcmp(arg, "-schema")) ||
        	         (!strcmp(arg, "--schema"))) {
        	    schema = arg;
        	    noent++;
        #endif
        #ifdef LIBXML_SCHEMATRON_ENABLED +/
        	}
    	         }
    }:"")
    ~ (!defined!"LIBXML_SCHEMATRON_ENABLED" ? q{
    else if((!strcmp(arg,"-chkregister"))||(!strcmp(arg,"--chkregister")))
        static if (defined!"LIBXML_SCHEMAS_ENABLED")
        {
            {chkregister++;}
        }
        else
        {
        {chkregister++; /+ else if ((!strcmp(arg, "-schematron")) ||
        	         (!strcmp(arg, "--schematron"))) {
        	    schematron = arg;
        	    noent++;
        #endif +/
                }
        }
    }:"")
    ~ (defined!"LIBXML_SCHEMAS_ENABLED" ? q{
    else if((!strcmp(arg,"-relaxng"))||(!strcmp(arg,"--relaxng"))){relaxng=arg;noent++;}else if((!strcmp(arg,"-schema"))||(!strcmp(arg,"--schema")))
        static if (defined!"LIBXML_SCHEMATRON_ENABLED")
        {
            {schema=arg;noent++;}
        }
        else
        {
        {schema=arg;noent++;}
        }
    }:"")
    ~ (defined!"LIBXML_SCHEMATRON_ENABLED" ? q{
    else if((!strcmp(arg,"-schematron"))||(!strcmp(arg,"--schematron"))){schematron=arg;noent++;}
    }:"")
    ~ q{
     else if ((!strcmp(arg, "-nonet")) ||
                       (!strcmp(arg, "--nonet"))) {
            } else if ((!strcmp(arg, "-nocompact")) ||
                       (!strcmp(arg, "--nocompact"))) {
    	} else if ((!strcmp(arg, "-load-trace")) ||
    	           (!strcmp(arg, "--load-trace"))) {
    	    load_trace++;
            } else if ((!strcmp(arg, "-path")) ||
                       (!strcmp(arg, "--path"))) 
                       static if (defined!"LIBXML_XPATH_ENABLED")
                       {
                           {
        /+ #ifdef LIBXML_XPATH_ENABLED +/
                }
                       }
                       else
                       {
        { /+ else if ((!strcmp(arg, "-xpath")) ||
                           (!strcmp(arg, "--xpath"))) {
        	    noout++;
        	    xpathquery = arg;
        #endif +/
        	}
                       }
    }
    ~ (defined!"LIBXML_XPATH_ENABLED" ? q{
    else if((!strcmp(arg,"-xpath"))||(!strcmp(arg,"--xpath"))){noout++;xpathquery=arg;}
    }:"")
    ~ q{
     else {
    	    return;
    	}
}
);}

