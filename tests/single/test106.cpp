...
typedef const int(*git_commit_parent_callback)(size_t idx, void *payload);
...
static int commit_parent_from_varargs(size_t curr, void *payload)
{
	commit_parent_varargs *data = (commit_parent_varargs *)payload;
	int commit;
	printf("commit_parent_from_varargs curr=%d data->total=%d\n", curr, data->total);
	if (curr >= data->total)
		return 0;
	commit = va_arg(data->args, int);
	...
	...)
{
	...
	git_commit__create_internal(
		(git_commit_parent_callback)commit_parent_from_varargs, &data);
