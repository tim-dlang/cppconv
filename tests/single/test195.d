...
{
    return mixin(interpolateMixin(q{(cast(imported!q{test195}.CPpmd_State*) $(imported!q{test195}.Ppmd8_GetPtr(q{($(p2))}, q{(($(ctx3)).Stats)})))}));
};
}
...
{
    return mixin(interpolateMixin(q{$(imported!q{test195}.Ppmd8_GetStats(q{p}, q{$(ctx2)}))}));
};
