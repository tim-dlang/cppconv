...
struct SimpleAppender(T)
{
    T data_;
    size_t length;
    void put(const T s)
    {
        size_t needed = length + s.length;
        if (needed > data_.length)
        {
            size_t nextLength = 16;
            while (nextLength < needed)
                nextLength *= 2;
            data_.length = nextLength;
        }
        data_[length .. length + s.length] = s;
        length += s.length;
    }

    void put(const typeof(T.init[0]) s)
    {
...
string[] splitComma(string code)
{
    SimpleAppender!(string[]) app;
    size_t parenCount;
    size_t start;
    size_t i;
...
template octalHelper(ulong dec)
{
    enum ulong value = () {
        ulong d = dec;
        ulong o;
    ...
    static if (value <= 0xffff_ffff)
        enum uint value2 = value;
    else
        enum ulong value2 = value;
}

enum octal(ulong dec) = octalHelper!dec.value2;

struct UnknownType(string desc)
{
    static assert(false, desc);
}

template UnresolvedMergeConflict(T...)
{
    static assert(false, "unresolved merge conflict " ~ T.stringof);
}

template staticString(T, string s)
{
    enum staticString = () {
        import core.internal.traits : Unqual;

        Unqual!T[s.length + 1] r;
...
template buildStaticArray(string type, string elements)
{
    enum buildStaticArray = () {
        auto parts = splitComma(elements);
        SimpleAppender!(char[]) app;
        app.put("(){");
        app.put(type);
        app.put("[");
        app.put(numberToString(parts.length));
        app.put("] r = [");
        foreach (i, x; parts)
        {
            app.put(x);
...
template buildStaticArray(string type, size_t size, string elements)
{
    enum buildStaticArray = () {
        auto parts = splitComma(elements);
        assert(parts.length <= size);
        SimpleAppender!(char[]) app;
        app.put("(){");
        app.put(type);
        app.put("[");
        app.put(numberToString(size));
        app.put("] r = [");
        foreach (i, x; parts)
        {
            app.put(x);
...
auto fallbackAssignExpression(string field, string op, L, R)(ref L lhs, R rhs)
{
    enum op2 = strip(op);
    enum field2 = strip(field);
    static assert(op2[$ - 1] == '=');
    mixin("return lhs." ~ field ~ " = lhs." ~ field ~ " " ~ op2[0 .. $ - 1] ~ " rhs;");
}

auto fallbackPostfixExpression(string field, string op, L)(ref L lhs)
{
    mixin("auto r = lhs." ~ field ~ ";\n" ~ "lhs." ~ field ~ " = lhs." ~ field ~ ((op == "++")
            ? " + 1" : (op == "--") ? " - 1" : "assert(false)") ~ ";\n" ~ "return r;");
}

auto fallbackUnaryExpression(string field, string op, L)(ref L rhs)
{
    mixin("rhs." ~ field ~ " = rhs." ~ field ~ ((op == "++") ? " + 1" : (op == "--")
            ? " - 1" : "assert(false)") ~ ";\n" ~ "return rhs." ~ field ~ ";");
}

template wchar_literal(string str)
{
    enum immutable(wchar_t)[] wchar_literal = str;
}

typeof(T.init[0])[n] castStaticArray(T, S, size_t n)(S[n] data)
{
    return cast(typeof(T.init[0])[n]) data;
}

struct ValueClass(C) if (is(C == class))
{
    void*[(__traits(classInstanceSize, C) + (void*).sizeof - 1) / (void*).sizeof] classData;
    C toRealClass()
    {
        return cast(C) classData.ptr;
    }

    alias toRealClass this;
    @disable this();
    @disable this(this);
}
